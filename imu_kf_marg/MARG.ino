...
#define gyroMeasDrift     3.14159265358979f * (0.01f /180.0f)    // gyroscope measurement error in rad/s/s (shown as 0.2f deg/s/s)
#define beta sqrt(3.0f / 4.0f) * gyroMeasError           // compute beta
#define zeta sqrt(3.0f / 4.0f) * gyroMeasDrift           // compute zeta

/*
* MARG filter implementation
*/
class MARG 
{
  public:
    ...
    void Update_quaternion_init(float X, float Y, float Z)
    {
      float c1 = cos(Z*DEG_TO_RAD * 0.5);
      float c2 = cos(Y*DEG_TO_RAD * 0.5);
      float c3 = cos(X*DEG_TO_RAD * 0.5);
      float s1 = sin(Z*DEG_TO_RAD * 0.5);
      float s2 = sin(Y*DEG_TO_RAD * 0.5);
      float s3 = sin(X*DEG_TO_RAD * 0.5);

      SEq_2 = c1*c2*s3 - s1*s2*c3;
      SEq_3 = c1*s2*c3 + s1*c2*s3;
      SEq_4 = s1*c2*c3 - c1*s2*s3;
      SEq_1 = c1*c2*c3 + s1*s2*s3;
    }
    ...
    */
    float getX() 
    { 
      return (float) (atan2(2 * (SEq_1 * SEq_2 + SEq_3 * SEq_4),1 - 2 * (SEq_2 * SEq_2 + SEq_3 * SEq_3))*RAD_TO_DEG); 
    }; 
    
    float getY() 
    { 
      return (float) (asin(2 * (SEq_1 * SEq_3 - SEq_4 * SEq_2))*RAD_TO_DEG);
    }; 
    
    float getZ() 
    { 
      return (float) (atan2(2 * (SEq_1 * SEq_4 + SEq_2 * SEq_3) , 1 - 2* (SEq_3 * SEq_3 + SEq_4 * SEq_4))*RAD_TO_DEG);
    }; 
    
    
    
    
    void SetSEq_1(float q) 
    { 
      SEq_1=q;  
    };    
    void SetSEq_2(float q) 
    { 
      SEq_2=q;  
    };
    void SetSEq_3(float q) 
    { 
      SEq_3=q;  
    };
    void SetSEq_4(float q) 
    { 
      SEq_4=q;  
    };
